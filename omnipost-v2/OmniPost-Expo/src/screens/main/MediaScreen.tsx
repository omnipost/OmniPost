...
// NOTE: react-native-image-crop-picker replaced with expo-image-picker
// NOTE: react-native-fast-image replaced with standard Image
...
export default function MediaScreen() {
  ...
  function handleDeleteSelected() {
    Alert.alert('Delete Files', `Delete ${selected.length} file${selected.length > 1 ? 's' : ''}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: () => { setSelected([]); Toast.show({ type: 'success', text1: `${selected.length} files deleted` }); } },
    ]);
  }
  ...
  async function handleUpload() {
    ...
    if (!result.canceled && result.assets.length > 0) {
      Toast.show({ type: 'success', text1: `${result.assets.length} file(s) selected (demo mode)` });
    }
  }
  ...
  const renderGridItem = ({ item: asset }: { item: MediaAsset }) => {
    ...
    return (
      <TouchableOpacity onPress={() => toggleSelect(asset.id)} activeOpacity={0.85}
        style={[s.gridItem, isSel && s.gridItemSelected]}>
        <Image source={{ uri: asset.thumbUrl ?? asset.url }} style={s.gridThumb} />
        {asset.type === 'video' && (
          <View style={s.videoOverlay}>
            <Text style={s.videoIcon}>▶</Text>
            {asset.duration && (
              <Text style={s.videoDuration}>{Math.floor(asset.duration / 60)}:{String(asset.duration % 60).padStart(2, '0')}</Text>
            )}
          </View>
        )}
        {isSel && <View style={s.checkOverlay}><Text style={s.checkIcon}>✓</Text></View>}
      </TouchableOpacity>
    );
  };

  const renderListItem = ({ item: asset, index }: { item: MediaAsset; index: number }) => (
    <View style={[s.listRow, index < filtered.length - 1 && s.listRowBorder]}>
      <Image source={{ uri: asset.thumbUrl ?? asset.url }} style={s.listThumb} />
      <View style={s.listMeta}>
        <Text style={s.listName} numberOfLines={1}>{asset.filename}</Text>
        <Text style={s.listSub}>{fmtSize(asset.size)} · {asset.type}</Text>
        {asset.width && asset.height && <Text style={s.listDim}>{asset.width} × {asset.height}px</Text>}
      </View>
      <Badge label={asset.type} variant={asset.type === 'video' ? 'info' : asset.type === 'audio' ? 'brand' : 'default'} />
    </View>
  );
  ...
  return (
    <View style={s.root}>
      <View style={s.header}>
        <Text style={s.headerTitle}>{MOCK_MEDIA.length} files · {fmtSize(totalBytes)}</Text>
        <View style={s.headerActions}>
          {selected.length > 0 && (
            <TouchableOpacity onPress={handleDeleteSelected} style={s.deleteBtn}>
              <Text style={s.deleteBtnText}>🗑 Delete ({selected.length})</Text>
            </TouchableOpacity>
          )}
          <TouchableOpacity onPress={handleUpload} style={s.uploadBtn}>
        ...
        <View style={s.content}>
          <Card style={{ marginBottom: 14 }}>
            <View style={s.storageRow}>
              <Text style={s.storageLabel}>Storage used</Text>
              <Text style={s.storageValue}>{fmtSize(totalBytes)} / {(limitMb / 1024).toFixed(0)} GB</Text>
            </View>
            <View style={s.storageBar}><View style={[s.storageFill, { width: `${Math.min(100, usedPct)}%` as any }]} /></View>
            <Text style={s.storagePlan}>Creator plan · 5 GB included</Text>
          </Card>
          ...
          <View style={s.toolbar}>
            ...
            <View style={s.viewToggle}>
              {[['grid', '⊞'], ['list', '☰']].map(([v, icon]) => (
                <TouchableOpacity key={v} onPress={() => setView(v as 'grid' | 'list')} style={[s.viewBtn, view === v && s.viewBtnActive]}>
                  <Text style={[s.viewIcon, view === v && s.viewIconActive]}>{icon}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>

          <TouchableOpacity onPress={handleUpload} style={s.dropZone}>
            <Text style={s.dropIcon}>⬆️</Text>
            <Text style={s.dropTitle}>Tap to upload files</Text>
            <Text style={s.dropSub}>Images, videos, audio · Up to 500 MB</Text>
          </TouchableOpacity>

          {filtered.length === 0 ? (
            <EmptyState icon="📁" title="No media found" description="Upload your first file."
              action={<Button label="Upload Media" onPress={handleUpload} style={{ marginTop: 4 }} />} />
          ) : view === 'grid' ? (
            <FlatList data={filtered} renderItem={renderGridItem} keyExtractor={item => item.id}
              numColumns={3} columnWrapperStyle={s.gridRow} scrollEnabled={false} contentContainerStyle={{ gap: 6 }} />
          ) : (
            <Card style={{ padding: 0, overflow: 'hidden' }}>
              <FlatList data={filtered} renderItem={renderListItem} keyExtractor={item => item.id} scrollEnabled={false} />
            </Card>
          )}
